...
class CalculateMealNutrients(
    ...
    operator fun invoke(trackedFood: List<TrackedFood>): Result {
        ...
        private fun bmr(userInfo: UserInfo): Int {
            val userHeight = (userInfo.heightFeet * 30.48f) + (userInfo.heightInches * 2.54f)
            return when(userInfo.gender) {
                is Gender.Male -> {
                    (66.47f + 13.75f * userInfo.weight +
                            5f * userHeight - 6.75f * userInfo.age).roundToInt()
                }
                is Gender.Female ->  {
                    (665.09f + 9.56f * userInfo.weight +
                            1.84f * userHeight - 4.67 * userInfo.age).roundToInt()
                }
            }
        }
        ...
        private fun dailyCaloryRequirement(userInfo: UserInfo): Int {
            val activityFactor = when(userInfo.activityLevel) {
                ...
                is ActivityLevel.Medium -> 1.45f
                is ActivityLevel.High -> 1.725f
            }
